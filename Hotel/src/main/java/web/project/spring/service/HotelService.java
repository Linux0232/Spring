...
public interface HotelService {
	...
	public abstract List<HotelVO> readOrderByStar(PageCriteria c);
	// 리뷰순 정렬기준으로 전체 호텔list 가져옴
	public abstract List<HotelVO> readOrderByReviewCount(PageCriteria c);
	// 최저가격순 정렬기준으로 전체 호텔list 가져옴
	public abstract List<HotelVO> readOrderByLowPrice(PageCriteria c);
	// 최고가격순 정렬기준으로 전체 호텔list 가져옴
	public abstract List<HotelVO> readOrderByHighPrice(PageCriteria c);
	// hotel_no의 전체 정보를 가져옴
	public abstract HotelVO readHotelInfo(int hotel_no);
	// 조건(체크인,체크아웃,호텔번호,인원수)에 따른 detail페이지 hotel_room 검색결과
	public abstract List<RoomVO> readDetailForRoom(String check_in, String check_out, int hotel_no, int person);
	...
	// 호텔의 좋아요 개수
	public abstract int readLikeCount(int hotel_no);
}
